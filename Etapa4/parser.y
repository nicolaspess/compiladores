...
%%
programa : listaDeDeclaracoes { astPrint($1, 0);  printSource($1); set_declarations($1); }
         ;
...
declaracoes : tipo TK_IDENTIFIER '=' literal ';'                        { $$ = astCreate(AST_DEC, $2, $1, $4, 0, 0); }
            ...
            | tipo TK_IDENTIFIER '[' literal ']' inicializacao ';'          { $$ = astCreate(AST_DECARR, $2, $1, $4, $6, 0); }
...
inicializacao : ':' inicializacao2        { $$ = astCreate(AST_INI, 0, $2, 0, 0, 0); }
              | { $$ = 0; }
              ;
...
fimDeParametros : ',' parametro fimDeParametros { $$ = astCreate(AST_FPARAM, 0, $2, $3, 0, 0); }
                | { $$ = 0; }
                ; 
...
controleFluxo : KW_IF '(' expressao ')' KW_THEN comando                   { $$ = astCreate(AST_IFT, 0, $3, $6, 0, 0); }
              ...
              | KW_IF '(' expressao ')' KW_THEN comando KW_ELSE comando { $$ = astCreate(AST_IFTE, 0, $3, $6, $8, 0); }         
	      ;

atribuicao : TK_IDENTIFIER '=' expressao                    { $$ = astCreate(AST_ASS, $1, $3, 0, 0, 0); }
           | '#' TK_IDENTIFIER '=' expressao                { $$ = astCreate(AST_ASSPT1, $2, $4, 0, 0, 0); }
           | '&' TK_IDENTIFIER '=' expressao                { $$ = astCreate(AST_ASSPT2, $2, $4, 0, 0, 0); }
